...
def res(amount,period,rate,rate1,repayment):
    ...
    if repayment == '0':
        ...
        for t in range(2, n + 1):
            ...
            bi = b - ci
            if t < n:
                ssss.append("第%d个月应还利息为%s,应还本金为%s,还款总额(本金+利息)为%s,剩余本金%s" % (t, round(ci, 2), round(bi, 2), round(b, 2), round(a1 - bi, 2)))
            else:
                ssss.append("第%d个月应还利息为%s,应还本金为%s,还款总额(本金+利息)为%s,剩余本金0" % (t, round(ci, 2), round(bi, 2), round(b, 2)))
            a1 = a1 - bi
